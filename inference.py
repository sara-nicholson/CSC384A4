...
class ParticleFilter(InferenceModule):
	...
	for i in range(len(self.legalPositions)):
	    ...
	    if i < remainder:
		count += 1
	    self.particles.extend([self.legalPositions[i] for j in range(count)])
	    self.beliefs[self.legalPositions[i]] = float(count)/float(self.numParticles)
	...
	for i in range(self.numParticles):
	    resample.append(util.sample(self.beliefs))
        for p in resample: 
	    trueDistance = util.manhattanDistance(p, pacmanPosition)
            if emissionModel[trueDistance] > 0:
                weights[p] = self.beliefs[p]*emissionModel[trueDistance]
	...
	#    self.beliefs[i] = self.particles[i]/sum(self.particles)
...
class JointParticleFilter:
	...
	for i in range(len(possiblePos)):
	    count =self.numParticles/len(possiblePos)
	    remainder = self.numParticles % len(possiblePos)
	    if i < remainder:
		count += 1
	    #print count
	    self.particles.extend([possiblePos[i] for j in range(count)])
	...
	for p in self.particles:
		...
		if noisyDistances[i] == None:
		    ...
		    temp = temp*emissionModels[i][trueDist]
		    
	    weights[p] = temp*self.beliefs[p]
	if weights.totalCount() == 0:
	    self.initializeParticles()
	    for i in range(self.numGhosts):
		if noisyDistances[i] == None:
		    for j in range(len(self.particles)):
			newPart = self.getParticleWithGhostInJail(self.particles[j],i)
			self.particles[j] = newPart
		self.getBeliefDistribution()
       	else:
	    weights.normalize()	
	    dist = weights.values()
	    vals = weights.keys()
	    resample = util.nSample(dist,vals,self.numParticles)
	    #for i in range(self.numParticles):
	...
	possiblePos = list(itertools.product(self.legalPositions,repeat=self.numGhosts))
	    ...
	    count = self.particles.count(i)
	    self.beliefs[i] = float(count)/float(self.numParticles)    
	#print type(self.beliefs)
	return self.beliefs
	#util.raiseNotDefined()
        "*** END YOUR CODE HERE ***"
